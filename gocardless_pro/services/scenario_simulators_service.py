...
<li>`payout_create`: Creates a payout containing payments in `confirmed`, `failed` & `charged_back` states; refunds in `submitted` & `bounced`; and all related fees. Can only be used with a positive total payout balance and when some eligible items exist.</li>
