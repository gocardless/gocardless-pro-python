# WARNING: Do not edit by hand, this file was generated by Crank:
#
#   https://github.com/gocardless/crank
#

from . import base_service
from .. import resources

class MandatePdfsService(base_service.BaseService):
    """Service class that provides access to the mandate_pdfs
    endpoints of the GoCardless Pro API.
    """

    RESOURCE_CLASS = resources.MandatePdf
    RESOURCE_NAME = 'mandate_pdfs'

    def create(self, params=None):
        """Generate a mandate PDF.

        Generates a temporary URL for viewing a PDF mandate.
        
       
        Bank account details may be supplied using an IBAN (international bank
        account number), or [local details](#ui-local-bank-details), or
        alternatively you can provide the ID of an existing
        [mandate](#core-endpoints-mandates) to create a PDF of.
        
      
         To generate a mandate in a foreign language, set your
        `Accept-Language` header to the relevant [ISO
        639-1](http://en.wikipedia.org/wiki/List_of_ISO_639-1_codes#Partial_ISO_639_table)
        language code. Currently Dutch, English, French, German, Italian,
        Portuguese and Spanish are supported.

        Args:
          params (dict, optional): Request body.

        Returns:
          MandatePdf
        """
        path = '/mandate_pdfs'
        response = self._perform_request('POST', path, params)
        return self._resource_for(response)

