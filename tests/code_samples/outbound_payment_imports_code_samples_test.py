...
@responses.activate
def test_outbound_payment_imports_create_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/outbound_payment_imports'
    ...
    response_body = { 'outbound_payment_imports': {} }
    for _ in range(5):
        responses.add(
            'POST',
            url_pattern,
...
@responses.activate
def test_outbound_payment_imports_get_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/outbound_payment_imports/:identity'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'outbound_payment_imports': {} }
    for _ in range(5):
...
@responses.activate
def test_outbound_payment_imports_list_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/outbound_payment_imports'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'outbound_payment_imports': [{}], 'meta': { 'cursors': {'after': None, 'before': None}, 'limit': 50 } }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
