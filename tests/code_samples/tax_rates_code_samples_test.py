...
@responses.activate
def test_tax_rates_list_code_sample():
    ...
    response_body = { 'tax_rates': [{}], 'meta': { 'cursors': {'after': None, 'before': None}, 'limit': 50 } }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
...
@responses.activate
def test_tax_rates_get_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/tax_rates/:identity'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'tax_rates': {} }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
