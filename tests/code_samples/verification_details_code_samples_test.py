...
@responses.activate
def test_verification_details_create_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/verification_details'
    ...
    response_body = { 'verification_details': {} }
    for _ in range(5):
        responses.add(
            'POST',
            url_pattern,
...
@responses.activate
def test_verification_details_list_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/verification_details'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'verification_details': [{}], 'meta': { 'cursors': {'after': None, 'before': None}, 'limit': 50 } }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
