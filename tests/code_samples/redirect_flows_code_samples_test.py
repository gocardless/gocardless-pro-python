...
@responses.activate
def test_redirect_flows_get_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/redirect_flows/:identity'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'redirect_flows': {} }
    for _ in range(5):
...
@responses.activate
def test_redirect_flows_complete_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/redirect_flows/:identity/actions/complete'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'redirect_flows': {} }
    for _ in range(5):
