...
@responses.activate
def test_creditor_bank_accounts_create_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/creditor_bank_accounts'
    ...
    response_body = { 'creditor_bank_accounts': {} }
    for _ in range(5):
        responses.add(
            'POST',
            url_pattern,
...
@responses.activate
def test_creditor_bank_accounts_list_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/creditor_bank_accounts'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'creditor_bank_accounts': [{}], 'meta': { 'cursors': {'after': None, 'before': None}, 'limit': 50 } }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
...
@responses.activate
def test_creditor_bank_accounts_get_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/creditor_bank_accounts/:identity'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'creditor_bank_accounts': {} }
    for _ in range(5):
...
@responses.activate
def test_creditor_bank_accounts_disable_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/creditor_bank_accounts/:identity/actions/disable'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'creditor_bank_accounts': {} }
    for _ in range(5):
