...
@responses.activate
def test_payment_account_transactions_get_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/payment_account_transactions/:identity'
    ...
    response_body = { 'payment_account_transactions': {} }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
...
@responses.activate
def test_payment_account_transactions_list_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/payment_accounts/:identity/transactions'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'payment_account_transactions': [{}], 'meta': { 'cursors': {'after': None, 'before': None}, 'limit': 50 } }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
