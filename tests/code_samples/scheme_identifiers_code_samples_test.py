...
@responses.activate
def test_scheme_identifiers_create_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/scheme_identifiers'
    ...
    response_body = { 'scheme_identifiers': {} }
    for _ in range(5):
        responses.add(
            'POST',
            url_pattern,
...
@responses.activate
def test_scheme_identifiers_list_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/scheme_identifiers'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'scheme_identifiers': [{}], 'meta': { 'cursors': {'after': None, 'before': None}, 'limit': 50 } }
    for _ in range(5):
        responses.add(
            'GET',
            url_pattern,
...
@responses.activate
def test_scheme_identifiers_get_code_sample():
    # Convert :param placeholders to regex wildcards for flexible matching
    stub_url = '/scheme_identifiers/:identity'
    url_pattern = re.compile('https://api.gocardless.com' + re.sub(r':[\w]+', r'[^/]+', stub_url))
    # Mock response - repeat multiple times to handle code samples with multiple API calls
    response_body = { 'scheme_identifiers': {} }
    for _ in range(5):
